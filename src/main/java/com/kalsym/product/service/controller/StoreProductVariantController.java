...
public class StoreProductVariantController {
    ...
    @DeleteMapping(path = {"/bulk"}, name = "store-product-variants-delete-by-id", produces = "application/json")
    @PreAuthorize("hasAnyAuthority('store-product-variants-delete-by-id', 'all') and @customOwnerVerifier.VerifyStore(#storeId)")
    public ResponseEntity<HttpResponse> deleteStoreProductVariantsByBulk(HttpServletRequest request,
            @PathVariable String storeId,
            @PathVariable String productId,
